...
Only `git_repository` exists so far (`internal/provider/git_repository_resource.go`), and it's intentionally inert: Create/Update just mirror `url` into `id`, Read is a no-op passthrough, Delete does nothing. It does not yet resolve auth or talk to any host — it's a reference/placeholder resource per the "reference-only" model in DESIGN.md.

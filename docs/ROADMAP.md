...
### Phase 6: Provider auth and GitLab (2026-08-19 to 2026-08-21, current branch)
...
Only HTTPS token/basic auth exists (`internal/git/gogit/client.go`'s `authMethod` only produces `http.BasicAuth`; `execgit` has no SSH-specific handling either).
Most git hosts default to SSH remotes, so this is a functional gap, not a nice-to-have, for anyone whose repos aren't set up for HTTPS + token.

### 2. Hosting-API backend used only for read paths
...
### 3. GitLab diff reconstruction gaps
...
### 5. Host extensibility beyond GitHub/GitLab
...
### 5a. Branch tip changed upstream to something unrelated to the patch stack

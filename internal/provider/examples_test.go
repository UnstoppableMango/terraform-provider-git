...
// via ConfigDirectory, so the examples themselves are the single source of
// truth for both documentation and integration coverage.
//
// The example .tf files deliberately omit explicit `provider "git" {}`
// blocks and a required_providers entry for git: terraform-plugin-testing
// errors if a TestCase sets ProtoV6ProviderFactories and the ConfigDirectory
// also declares its own provider block for that name, and separately
// reattaches the in-process test provider under the default
// registry.terraform.io/hashicorp/<name> address, which a real source
// address (e.g. "UnstoppableMango/git") would not match. Real, published
// providers (github, random) don't have this constraint and are declared
// normally in examples/full/github/main.tf; ExternalProviders can't be
// combined with ConfigDirectory, so those are installed for real from the
// public registry during the test.
...
func TestAccExampleGitHubFull(t *testing.T) {
	...
	tfresource.Test(t, tfresource.TestCase{
		...
		Steps: []tfresource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("../../examples/full/github"),
